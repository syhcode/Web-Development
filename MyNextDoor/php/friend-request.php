...
if (isset($_SESSION["bid"]) ) {
    ...
    if ($stmt = $mysqli->prepare("SELECT uid, uname, address, profile, email FROM user, friend WHERE user.uid = friend.aid AND friend.bid=? AND friend.agree='N';")) {
        ...
        if ($stmt->num_rows > 0) {
            while ($stmt->fetch()) {
				...
				<div class="row col-sm-offset-1 col-sm-8 well">
					...
					<form id="friend-request" action="friend-success.php" method="post">
						<input type="hidden" name="bid" value="$personal_id">
					</form>
