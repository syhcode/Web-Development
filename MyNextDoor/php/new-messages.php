...
if (isset($_POST['email']) && isset($_POST['password'])) {
    ...
    if ($stmt = $mysqli->prepare("SELECT uid, time, uname, bid, address FROM user WHERE email=? AND password=?;")) {

        $stmt->bind_param("ss", $_POST['email'], $_POST['password']);
        $stmt->execute();

        $stmt->bind_result($uid, $time, $uname, $bid, $address);
        $stmt->store_result();
...
if ($stmt = $mysqli->prepare("SELECT tid, title, thread.time, user.uname, user.address FROM thread, user
	WHERE thread.uid=user.uid AND thread.time > ? ORDER BY thread.time DESC;")
) {

    $stmt->bind_param("s", $_SESSION["time"] );
    $stmt->execute();
