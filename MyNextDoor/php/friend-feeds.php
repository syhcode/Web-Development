...
<div class="row col-sm-offset-1 col-sm-8 well">
	...
	<form method="post" action="friend-feeds.php">
	<div>
...
if ($stmt = $mysqli->prepare("SELECT thread.tid, title, thread.time, user.uname, user.address FROM thread, user, receive
	WHERE thread.uid=user.uid AND ((scope='f' AND receive.tid=thread.tid AND receive.uid=?) OR user.uid=?) ORDER BY thread.time DESC;")
) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        while ($stmt->fetch()) {
            ...
            echo "<form method=\"post\" action=\"friend-feeds.php\">";
