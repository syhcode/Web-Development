...
if (isset($_POST["bid"]) && isset($_SESSION["uid"])) {

    //create SQL query
    if ($stmt = $mysqli->prepare("INSERT INTO friend(aid, bid, agree) VALUES (?, ?, ?);")) {
        $condition="Y";

        $stmt->bind_param("iis",  $_SESSION["uid"], $_POST["bid"], $condition );
        //execute SQL query
        $stmt->execute();
        $stmt->close();
    } else {
    ...
    if ($stmt = $mysqli->prepare("UPDATE friend SET agree='Y' WHERE aid=? AND bid=?;")) {

        $stmt->bind_param("ii", $_POST["bid"], $_SESSION["uid"]);
        //execute SQL query
        $stmt->execute();
        $stmt->close();
    } else {
		...
		<div class="container">
			<div class="well well-lg panel-group col-sm-offset-3 col-sm-6">
				<div class="panel panel-success">
					<div class="panel-heading">friend success!</div>
